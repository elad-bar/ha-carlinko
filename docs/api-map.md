...
Raw captures (`capture/flows.mitm`, dumps) were local-only and are **not** in this repo.

> Original notes contained live owner data (token, VIN, vehicle/device ids). Keep any
> private captures local; do not share unsanitised.
...
| ------ | ------------------------------------------------------ | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/pub/timestamp`                                       | Server time                                 | No auth. `data` = epoch ms. **Implemented** for signed-request clock skew.                                                                                                                                                                                                                                                                                                                                                                                                |
...
| GET    | `/netty/getConnect/2/{deviceSn}`                       | Get WS URL                                  | returns `ws://wss-cqr-sea.hzhjcl.com:4002`. **Implemented** in `ApiClient.get_ws_connect` (WS connect prefers this; region template fallback).                                                                                                                                                                                                                                                                                                                            |
...
| GET    | `/user/notice/unReadCount?vehicleId=`                  | Notification counts                         | Category tiles (`vehicleNoticeVo`, `controlNoticeVo`, …). Integration uses types **2** and **4** only. Optional `vehicleId` (global when omitted).                                                                                                                                                                                                                                                                                                                        |
| GET    | `/user/notice/page?page&size&type&vehicleId=`          | Notice list                                 | Row includes title/contents; no separate body GET. HA fires `carlinko_notice` for new operational rows. `vehicleId` optional.                                                                                                                                                                                                                                                                                                                                             |
| GET    | `/user/maintain/page?vehicleId=&queryKey=&page&size`   | Service history                             | Dealer maintain rows; summary sensors + `get_maintain_history` service.                                                                                                                                                                                                                                                                                                                                                                                                   |
| GET    | `/user/maintain/details/{maintainId}`                  | Service detail                              | Adds dealer/logList; `get_maintain_details` service. Signed params include `maintainId`.                                                                                                                                                                                                                                                                                                                                                                                  |
| GET    | `/user/higherFirmware?deviceId=&version=`              | Firmware offer check                        | Read-only in HA (`deviceId` from `/user/vehicle`). No start-upgrade / download.                                                                                                                                                                                                                                                                                                                                                                                           |
| GET    | **`/user/vehicle/state/{vehicleId}`**                  | **Telemetry over REST**                     | Same 73-byte hex blob as WS `action:6`. **Implemented** as WS-disconnected fallback poll. Verified live 2026-08-13 (byte-for-byte match with the WS frame, incl. battery/range). Reported by [@jebentancour](https://github.com/jebentancour) in [#5](https://github.com/GodrezJr2/j5-ev-dashboard/issues/5)                                                                                                                                                              |
...
| POST   | **`/maps/deviceLocate`**                               | **Vehicle GPS + address**                   | Body `{sn, showAddress:1, timestamp}`. Success `0000` → `{lat,lng,address}`. `50052` = query failed (often offline / no fix). `50049` = function unavailable. HA polls every 15 min when supported.                                                                                                                                                                                                                                                                       |
| POST   | **`/user/vehicle/remoteControl`**                      | **CONTROL the car**                         | body `{vehicleId, deviceSn, data:"<hexcmd>", timeOut:20}`. See control.                                                                                                                                                                                                                                                                                                                                                                                                   |

## Remote control

`POST /user/vehicle/remoteControl` body (note: request field is still named
`deviceSn`; the value is `/user/vehicle`.`deviceId`):
...
block (bytes ~44–51) stayed **`FF` through a full road drive** (slow laps _and_ road speed),
not just while parked. The official CarLinko app confirms it too — it shows tyres as "-.- bar".

The blob _has_ pressure/temp fields and the app's `vehicleControlConfig` even ships conversion
...
The decoder + formula are kept so a car which _does_ report real values would display them,
...
| `unlocked`                       | byte 3                      | **LOCK state: `0`=locked, `!=0`=unlocked.** VERIFIED ✅ — live lock/unlock test on the Omoda E5 (#5) _and_ J5 data: 217 parked 0→1 flips, and the byte lingers at 1 for a median 105 s after driving stops (the walk-away-and-lock delay). Previously mistaken for ignition |
...
| `volt12`                         | bytes 12–13 (BE u16 ×0.01)  | 13.84 V. **EMEA 2026-09-07:** same pair as `--seat-probe` `unknown_u16_a` (1300–1384 raw = 13.00–13.84 V)                                                                                                                                                                   |
...
| `seat_heat`                      | bytes 32–33, **34**, **36** | Front L=32, R=33. Rear L=**34** (`741702` live). Rear R=**36** (dash, EMEA 2026-09-07)                                                                                                                                                                                      |
| `seat_vent`                      | bytes 37–38, **39**, **41** | Front L=37, R=38. Rear L=**39** (inferred), R=**41** (`741E02` live, EMEA 2026-09-07)                                                                                                                                                                                       |
...
| `windshield_heat`                | byte 64                     | Front windshield heat on/off (dash: glass first, EMEA 2026-09-07)                                                                                                                                                                                                           |
| `steer_heat`                     | byte 65                     | Steering wheel heat on/off (dash: after windshield)                                                                                                                                                                                                                         |
| `wltc_range_km`                  | bytes 68–69 (BE u16)        | **rated (WLTC) range** — _not_ a mirror of EV range. See below                                                                                                                                                                                                              |
| headline range                   | bytes 70–71 (BE u16)        | EV range on a BEV (mirrors `range_km`); **fuel range** on a PHEV (652 km)                                                                                                                                                                                                   |
| `hv_state`                       | byte 5                      | HV/motor state per [#5](https://github.com/GodrezJr2/j5-ev-dashboard/issues/5) (`>=2` = on). E5 live data: **0=off, 1=low-voltage (15–90 s power on/off transition), 2=high-voltage/ready**. **EMEA 2026-09-07:** climate on/off moved this `0→2→0` (app label `engine`)    |

Offsets include the leading `77 00` header (same as CarLinko `BLOB` / engine `--seat-probe`).

**Seat heat/vent — live (EMEA, 2026-09-07):** dash driver/passenger vent off → bytes **37** / **38**; front heat 32/33. `--seat-test vent-rr` (`741E`) moved only **byte 41**. `--seat-test heat-lr` (`7417`) moved only **`seat_heat_B` (byte 34)**. Dash rear-right heat stepped **`seat_heat_D` (byte 36)** 3→2→1→0 independently of left. Rear L vent is **39** by elimination. Windshield heat is byte **64**, wheel heat byte **65** (glass on then off, then wheel). Cloud accepted rear opcodes while `RearHeater` / `RearVent` were false. App A/B/C/D physical names were wrong (C is front right).
...
Tyre scale and powertrain are available from this payload for `config.json` / `KNOWN_CAR_OVERRIDES` in `consts`.
...
The integration surfaces `charging.mode` (`ac`/`dc`), `charging.state`, `charging.remaining_min`
...
EV range there, a PHEV writes its _fuel_ range.
...
- Direct-TPMS PSI validation on a car that actually populates bytes 44–51 (J5 stays `FF`).

Reads can run off the WebSocket + token alone; REST signing is used for login, vehicle fetch,
