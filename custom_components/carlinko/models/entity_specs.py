...
class EntitySpec:
    ...
    commands: Mapping[str, str] = field(default_factory=dict)  # action → remoteControl hex
...
ENTITY_SPECS: tuple[EntitySpec, ...] = (
    # --- sensors (always) ---
    EntitySpec(
        key="battery", platform="sensor", name="Battery",
        data_path="battery", unit="%", device_class="battery", state_class="measurement",
    ),
    EntitySpec(
    ...
    EntitySpec(
        ...
        data_path="volt12_calculated", unit="V", device_class="voltage", state_class="measurement",
    ),
    EntitySpec(
        key="volt12_status", platform="sensor", name="12V status",
        data_path="volt12_status", device_class="enum",
        options=("ok", "low", "critical"),
    ),
    EntitySpec(
        key="charge_power", platform="sensor", name="Charge Power",
        resolve="charge_power", unit="kW", device_class="power", state_class="measurement",
    ),
    EntitySpec(
        key="consumption", platform="sensor", name="Consumption",
        data_path="consumption_calculated", unit="kWh/100km", icon="mdi:lightning-bolt",
        state_class="measurement",
    ),
    EntitySpec(
        key="charge_remaining", platform="sensor", name="Charge remaining",
        data_path="charging.remaining_min", unit="min", device_class="duration",
        state_class="measurement",
    ),
    EntitySpec(
        key="charge_mode", platform="sensor", name="Charge mode",
        data_path="charging.mode", device_class="enum",
        options=("none", "ac", "dc"), icon="mdi:ev-plug-type2",
    ),
    EntitySpec(
        key="charge_state", platform="sensor", name="Charge state",
        resolve="charge_state", device_class="enum",
    ...
    EntitySpec(
        ...
        resolve="hv_state", device_class="enum",
        options=("off", "lv", "ready", "unknown"), icon="mdi:car-electric",
    ),
    EntitySpec(
    ...
    EntitySpec(
        key="charging", platform="binary_sensor", name="Charging",
        data_path="charging.active", device_class="battery_charging",
    ),
    EntitySpec(
        key="online", platform="binary_sensor", name="Online",
        data_path="online", device_class="connectivity",
    ),
    EntitySpec(
        key="moving", platform="binary_sensor", name="Moving",
        data_path="moving", device_class="moving",
    ),
    EntitySpec(
        key="tyres_ok", platform="binary_sensor", name="Tyre problem",
        resolve="tyres_problem", device_class="problem",
    ),
    EntitySpec(
        key="door", platform="binary_sensor", name="Any door",
        resolve="door_any", device_class="door",
    ),
    EntitySpec(
        key="door_driver", platform="binary_sensor", name="Driver door",
        resolve="door_bit", door_bit=1, device_class="door",
    ),
    EntitySpec(
        key="door_passenger", platform="binary_sensor", name="Passenger door",
        resolve="door_bit", door_bit=2, device_class="door",
    ),
    EntitySpec(
        key="door_rear_left", platform="binary_sensor", name="Rear left door",
        resolve="door_bit", door_bit=4, device_class="door",
    ),
    EntitySpec(
        key="door_rear_right", platform="binary_sensor", name="Rear right door",
        resolve="door_bit", door_bit=8, device_class="door",
    ),
    EntitySpec(
        key="seat_heat_left", platform="binary_sensor", name="Seat heat left",
        data_path="seat_heat_l", device_class="heat",
    ),
    EntitySpec(
        key="seat_heat_right", platform="binary_sensor", name="Seat heat right",
        data_path="seat_heat_r", device_class="heat",
    ),
    EntitySpec(
        key="seat_vent_left", platform="binary_sensor", name="Seat vent left",
        data_path="seat_vent_l", icon="mdi:car-seat",
    ),
    EntitySpec(
        key="seat_vent_right", platform="binary_sensor", name="Seat vent right",
        data_path="seat_vent_r", icon="mdi:car-seat",
    ),
    EntitySpec(
        key="defrost", platform="binary_sensor", name="Defrost",
        data_path="defrost_front", icon="mdi:car-defrost-front",
    ),
    # --- PHEV ---
    EntitySpec(
        key="fuel", platform="sensor", name="Fuel",
        data_path="fuel.pct", unit="%", icon="mdi:gas-station", state_class="measurement",
        when="phev",
    ),
    EntitySpec(
    ...
    EntitySpec(
        ...
        data_path="fuel.l_100", unit="L/100km", icon="mdi:fuel", state_class="measurement",
        when="phev",
    ),
    # --- direct TPMS ---
    EntitySpec(
    ...
    # --- config numbers ---
    EntitySpec(
        key="tariff", platform="number", name="Charging tariff",
        data_path="config:tariff", icon="mdi:currency-usd", config_key="tariff",
    ),
    EntitySpec(
        key="petrol_price", platform="number", name="Petrol price",
        data_path="config:petrol_price", icon="mdi:gas-station", config_key="petrol_price",
    ),
    EntitySpec(
        key="petrol_kml", platform="number", name="Petrol economy",
        data_path="config:petrol_kml", unit="km/L", icon="mdi:car-speed-limiter",
        config_key="petrol_kml",
    ),
    # --- body / climate state (commands gated by caps) ---
    EntitySpec(
        key="lock", platform="lock", name="Lock",
        resolve="lock_state", when="cap:lock",
        commands={
            "lock": "740100",
            "unlock": "740200",
            "locked": "740100",
    ...
    EntitySpec(
        key="climate", platform="climate", name="Climate",
        resolve="climate_mode", when="cap:ac.switch",
    ...
    EntitySpec(
        ...
        when="cap:find", commands={"press": "740400"},
    ),
    EntitySpec(
        key="charge_stop", platform="button", name="Stop charging",
        when="cap:charging", commands={"press": "742701"},
    ),
    EntitySpec(
        key="engine", platform="switch", name="Engine",
        data_path="engine_on", when="cap:engine",
        commands={"on": "740700", "off": "740800"},
    ),
    EntitySpec(
        key="engine_on", platform="binary_sensor", name="Engine on",
        data_path="engine_on", when="cap:engine",
    ),
    EntitySpec(
        key="gear", platform="select", name="Gear",
        options=("low", "high"), when="cap:gear",
        commands={"low": "742600", "high": "742602"},
    ),
    EntitySpec(
        key="quick_cool", platform="button", name="Quick cool",
        when="cap:ac.rapidCool", commands={"press": "742001"},
    ),
    EntitySpec(
        key="quick_heat", platform="button", name="Quick heat",
        when="cap:ac.rapidHeat", commands={"press": "741F01"},
    ),
    EntitySpec(
        key="defrost_cmd", platform="switch", name="Defog",
        data_path="defrost_front", when="cap:ac.defog",
        commands={"on": "741201", "off": "741200"},
    ),
    EntitySpec(
        key="purify", platform="switch", name="Air purify",
        when="cap:ac.purify",
    ...
    EntitySpec(
        key="seat_heatLR", platform="select", name="Rear L seat heat",
        options=("off", "L1", "L2", "L3"), when="cap:seats.heatLR",
        commands={"off": "741700", "L1": "741701", "L2": "741702", "L3": "741703"},
    ),
    EntitySpec(
        key="seat_ventLR", platform="select", name="Rear L seat vent",
        options=("off", "L1", "L2", "L3"), when="cap:seats.ventLR",
        commands={"off": "741C00", "L1": "741C01", "L2": "741C02", "L3": "741C03"},
    ),
    EntitySpec(
        key="seat_heatRR", platform="select", name="Rear R seat heat",
        options=("off", "L1", "L2", "L3"), when="cap:seats.heatRR",
        commands={"off": "741900", "L1": "741901", "L2": "741902", "L3": "741903"},
    ),
    EntitySpec(
        key="seat_ventRR", platform="select", name="Rear R seat vent",
        options=("off", "L1", "L2", "L3"), when="cap:seats.ventRR",
