    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            if region not in KNOWN_REGIONS:
                _LOGGER.warning(
                    f"config flow failed step=user error=invalid_region region={region!r}"
                )
                errors["base"] = "unknown"
    ...
    async def async_step_reauth_confirm(
        ...
        if user_input is not None:
            try:
                ...
                _LOGGER.error(
                    f"config flow failed step=reauth_confirm error=missing_region "
                    f"entry_id={partial_id(entry.entry_id)}: {err}"
                )
    ...
    async def _validate_login(
        ...
        *,
        ...
        if require_vehicles:
            ...
            _LOGGER.debug(
                "config flow _validate_login skip async_list_vehicles (reauth)"
            )
